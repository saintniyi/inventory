    ...
    namespace InventoryAPI.Data
    {
        public class SeedData
        {
            ...
            private async Task<byte[]?> DownloadImageAsync(string imageUrl)
            {
                ...
                {
                    _logger.LogWarning($"Failed to download image from {imageUrl}: {ex.Message}");
                    return null;
                }
            }
            ...
            private string GenerateImageUrl(string keyword)
            {
                // Placehold.co version
                // return $"https://placehold.co/600x400/000000/FFFFFF?text={Uri.EscapeDataString(keyword)}";

                // OR for real category images via LoremFlickr:
                return $"https://loremflickr.com/600/400/{Uri.EscapeDataString(keyword)}";
            }
            ...
            private async Task SeedProductsAsync()
            {
                ...
                var products = new List<Product>
                {
                    new Product
                    {
                        ...
                        ProductImage = await DownloadImageAsync(GenerateImageUrl("Smartphone"))
                    },
                    new Product
                    {
                        Name = "Men's Jacket",
                        ...
                        ProductImage = await DownloadImageAsync(GenerateImageUrl("Jacket"))
                    },
                    new Product
                    {
                        ...
                        ProductImage = await DownloadImageAsync(GenerateImageUrl("Rice"))
                    },
                    new Product
                    {
                        ...
                        ProductImage = await DownloadImageAsync(GenerateImageUrl("Chair"))
                    },
                    new Product
                    {
                        ...
                        ProductImage = await DownloadImageAsync(GenerateImageUrl("Book"))
                    }
                };
